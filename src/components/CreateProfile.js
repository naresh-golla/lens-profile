...
import { get, isArray, map, forEach,isEmpty, isNil } from "lodash";
      ...
      if(handle.length === 0){        
        ...
        try {
          ...
          if(!isNil(fileUrl)){
            ...
            profilePicUrl = "https://gateway.pinata.cloud/ipfs/" + response.data.IpfsHash;
          }
            ...
            try{
              ...
              if(txHash){
                openSuccessNotification("Profile Created Succesfully",txHash)
                setSpinner(false)
              ...
              openErrorNotification("Error Profile not created",error.message)
            }          
        } catch (error) {
          setSpinner(false)
        }
